...
void Extruder::manageTemperatures() {
    ...
    for (uint8_t controller = 0; controller < NUM_TEMPERATURE_LOOPS; controller++) {
        ...
        if (controller == autotuneIndex)
            continue; // Ignore heater we are currently testing
        ...
        if (controller < NUM_EXTRUDER) {
            ...
            if (controller == 1 && autotuneIndex != 0 && autotuneIndex != 1)
                if (tempController[0]->currentTemperatureC < EXTRUDER_FAN_COOL_TEMP && tempController[0]->targetTemperatureC < EXTRUDER_FAN_COOL_TEMP && tempController[1]->currentTemperatureC < EXTRUDER_FAN_COOL_TEMP && tempController[1]->targetTemperatureC < EXTRUDER_FAN_COOL_TEMP)
...
void Extruder::initExtruder() {
    ...
    for (i = 0; i < NUM_EXTRUDER; ++i) {
        ...
        if (act->enablePin > -1) {
            ...
            if (!act->enableOn)
                HAL::digitalWrite(act->enablePin, HIGH);
        }
...
void Extruder::selectExtruderById(uint8_t extruderId) {
    ...
    if (extruderId >= NUM_EXTRUDER)
        extruderId = Extruder::current->id;
    ...
    if (Printer::isAxisHomed(X_AXIS))
        dOffset[X_AXIS] = (extruder[extruderId].offsetMM[X_AXIS] - Extruder::current->offsetMM[X_AXIS]) * Printer::axisStepsPerMM[X_AXIS]; //neu - alt -> geht negativ als bewegung rein.
    if (Printer::isAxisHomed(Y_AXIS))
        dOffset[Y_AXIS] = (extruder[extruderId].offsetMM[Y_AXIS] - Extruder::current->offsetMM[Y_AXIS]) * Printer::axisStepsPerMM[Y_AXIS];
    if (Printer::isAxisHomed(Z_AXIS))
        dOffset[Z_AXIS] = (extruder[extruderId].offsetMM[Z_AXIS] - Extruder::current->offsetMM[Z_AXIS]) * Printer::axisStepsPerMM[Z_AXIS];
...
#ifdef EXTRUDER_MAX_TEMP
    if (temperatureInCelsius > EXTRUDER_MAX_TEMP)
        temperatureInCelsius = EXTRUDER_MAX_TEMP;
#endif // EXTRUDER_MAX_TEMP

    if (temperatureInCelsius < 0)
        temperatureInCelsius = 0;
    TemperatureController* tc = tempController[extr];
    if (tc->isSensorDefect() || tc->isSensorDecoupled())
        temperatureInCelsius = 0;
    if (tc->sensorType == 0)
        temperatureInCelsius = 0;
...
void Extruder::setTemperatureForAllExtruders(float temperatureInCelsius, bool beep) {
    for (uint8_t extrNr = 0; extrNr < NUM_EXTRUDER; extrNr++)
        Extruder::setTemperatureForExtruder(temperatureInCelsius, extrNr, beep);
}
...
const short* const temptables[15] PROGMEM = { (short int*)&temptable_1[0][0], (short int*)&temptable_2[0][0], (short int*)&temptable_3[0][0], (short int*)&temptable_4[0][0]
                                              ...
                                              ,
                                              (short int*)&temptable_8[0][0], (short int*)&temptable_9[0][0], (short int*)&temptable_10[0][0], (short int*)&temptable_11[0][0], (short int*)&temptable_12[0][0], (short int*)&temptable_13[0][0], (short int*)&temptable_14[0][0], (short int*)&temptable_15[0][0] };
...
void TemperatureController::setTargetTemperature(float target, float offset) {
    if (target <= 0.0f || target < targetTemperatureC)
        pwm_pos[pwmIndex] = 0;
...
void TemperatureController::waitForTargetTemperature(uint8_t plus_temp_tolerance) {
    ...
    while (true) {
        ...
        if (fabs(targetTemperatureC - currentTemperatureC) <= TEMP_TOLERANCE + plus_temp_tolerance)
            break;
        if (!dirRising && currentTemperatureC < MAX_ROOM_TEMPERATURE)
            break;
    }
...
TemperatureController heatedBedController = {
    ...
    HEATED_BED_PID_INTEGRAL_DRIVE_MIN,
