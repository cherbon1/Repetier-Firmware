...
bool SdBaseFile::open(SdBaseFile* dirFile, const uint8_t* dname, uint8_t oflag, bool bDir) {
    ...
    int8_t cVFATNeeded = -1, cVFATFoundCur = 0; //wIndex,
...
dir_t* SdBaseFile::getLongFilename(dir_t* dir, char* longFilename, int8_t cVFATNeeded, uint32_t* pwIndexPos) {
    ...
    uint8_t checksum = 0; // warning: 'checksum' may be used uninitialized in this function [-Wmaybe-uninitialized] in  checksum != lfn_checksum(dir->name)
...
uint8_t
spiRec(uint8_t* buf, uint16_t nbyte) {
...
bool Sd2Card::waitNotBusy(uint16_t timeoutMillis) {
    uint16_t t0 = HAL::timeInMilliseconds();
    while (spiRec() != 0XFF) {
        if (((uint16_t)HAL::timeInMilliseconds() - t0) >= timeoutMillis)
            goto fail;
    }
    return true;
...
void SdFile::write_P(FSTRINGPARAM(str)) {
    for (uint8_t c; (c = HAL::readFlashByte(str)); str++)
        write(c);
}
//------------------------------------------------------------------------------
